...
* [三、shell介绍](#三shell介绍)
  ...
  * [5、如何结束当前会话终端：](#5如何结束当前会话终端)
* [四、命令介绍](#四命令介绍)
  * [1、Linux命令](#1linux命令)
  * [2、Linux命令格式](#2linux命令格式)
  * [3、Linux命令相关的shell命令](#3linux命令相关的shell命令)
    * [type  \-\-显示命令的类别](#type----显示命令的类别)
    * [which－ 显示一个可执行程序的位置(只能找到bin目录下的命令）](#which-显示一个可执行程序的位置只能找到bin目录下的命 令)
...
* [五、文件和目录及操作](#五文件和目录及操作)
  ...
  * [重定向命令](#重定向命令)
    ...
    * [管道操作符|（竖杠）将前一个命令的标准输出作为后一个命令的标准输入](#管道操作符竖杠将前一个命令的标准输出作为后一 个命令的标准输入)
...
* [七、进程及操作](#七进程及操作)
  ...
  * [进程相关的shell命令](#进程相关的shell命令)
    * [ps \-\-（process status）命令用于显示当前进程的状态，类似于 windows 的任务管理器。](#ps---process-status命令用于 显示当前进程的状态类似于-windows-的任务管理器)
    * [top – 显示任务](#top--显示任务)
    * [jobs – 列出活跃的任务](#jobs--列出活跃的任务)
    * [bg – 把一个任务放到后台执行](#bg--把一个任务放到后台执行)
    * [fg – 把一个任务放到前台执行](#fg--把一个任务放到前台执行)
    * [kill – 给一个进程发送信号](#kill--给一个进程发送信号)
    * [killall – 杀死指定名字的进程](#killall--杀死指定名字的进程)
* [八、网络及操作](#八网络及操作)
* [七、编写shell程序](#七编写shell程序)
...
15 TERM 终止。这是 kill 命令发送的默认信号。如果程序仍然“活着”，可以接受信号，那么 这个
信号终止。
